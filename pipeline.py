...
def fetch_naver_financials(code, session):
        """NAVER 증권 재무정보 파싱 (영업현금흐름, CAPEX, 매출, 부채)"""
    import pandas as pd
    ...
    try:
                ...
                # 재무제표 데이터 (연간)
                fs_url = f"https://navercomp.wisereport.co.kr/v2/company/c1010001.aspx?cmp_cd={code}&cn="
                r = session.get(fs_url, headers=headers, timeout=10)
        ...
        for tbl in tables:
                        ...
                        for idx, row in tbl.iterrows():
                                            ...
                                            for v in row.iloc[1:]:
                                                                    try:
                                                                                                ...
                                                                                                if fv > 0:
                                                                                                                                vals.append(fv / 100.0)  # 억원 변환 (백만원 → 억원)
                    except:
                        pass
                                        if not vals:
                                                                continue
                    ...
                    if not rev_list:
                                                rev_list = vals
elif any(k in label for k in ["단기차입"]):
                    td_list = vals
elif any(k in label for k in ["장기차입", "사채"]):
                    ltd_list = vals
elif any(k in label for k in ["현금및현금성"]):
                    cash_list = vals
    ...
    for _, row in uni.iterrows():
        ...
        try:
                        ...
                        if cap_all is not None:
                                        ...
                                        fcf_info = f"fcf={fin['fcf_ttm']}" if fin['fcf_ttm'] else "fcf=null"
            print(f"  {code} {name} ok mktcap={mktcap:.0f}억 {fcf_info}")
            time.sleep(0.3)
except Exception as e:
